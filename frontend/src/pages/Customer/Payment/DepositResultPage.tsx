...
const DepositResultPage = () => {
  ...
  useEffect(() => {
    const syncResult = async () => {
      ...
      try {
        await walletService.recordDepositResult({
          orderCode,
          status: rawStatus,
        });
      } catch (e: unknown) {
  ...
  const content = useMemo(() => {
    if (uiStatus === "success") {
      return {
        ...
        description:
          "Bạn đã quay lại từ PayOS. Hệ thống sẽ cập nhật số dư ví Elingo ngay sau khi nhận webhook xác nhận thành công.",
      ...
      <div className="mx-auto w-full max-w-3xl">
        <div className="rounded-2xl border border-slate-200 bg-white shadow-lg overflow-hidden">
          <div className="bg-linear-to-r from-[#4da6ff] to-[#3d8fdd] px-6 py-5 text-white">
            ...
            <h2 className="text-xl md:text-2xl font-bold mt-1">Elingo Wallet</h2>
          </div>
            ...
            <div className="mt-6 rounded-xl border border-slate-200 p-4">
              ...
              {!loading && !error && (
                <p className="text-sm text-emerald-700">
                  Kết quả giao dịch đã được ghi nhận. Nếu trạng thái là thành công,
                  số dư ví sẽ được cập nhật sau bước xác nhận cuối.
                </p>
              )}
              {!loading && error && (
        ...
        <div className="mt-4 text-xs text-slate-500 px-1">
          Nếu đã chuyển tiền nhưng chưa thấy số dư thay đổi, vui lòng chờ thêm vài
          giây để hệ thống nhận xác nhận cuối từ cổng thanh toán.
        </div>
      </div>
    </div>
  );
